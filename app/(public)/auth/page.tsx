...
function AuthContent() {
  ...
  const next = searchParams.get('next') ?? '/pets'
  ...
  function handleLineLine() {
    const safeNext = next.startsWith('/') && !next.startsWith('//') ? next : '/pets'
    ...
    <main className="flex min-h-screen items-center justify-center bg-gradient-to-b from-orange-50 to-white px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="mb-8 flex flex-col items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Furchic</h1>
        ...
        <div className="rounded-2xl bg-white p-8 shadow-lg ring-1 ring-gray-100">
          ...
          {mode === 'login' && (
            <form onSubmit={loginForm.handleSubmit(handleLogin)} noValidate>
              <div className="space-y-4">
                ...
                >
                  {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : '登入'}
                </button>
              </div>
              ...
              <p className="mt-5 text-center text-sm text-gray-500">
                ...
                <button
                  type="button"
                  onClick={() => { setMode('register'); setGlobalError(null) }}
            ...
            <form onSubmit={registerForm.handleSubmit(handleRegister)} noValidate>
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center rounded-lg bg-[#E8820C] px-4 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-60"
                >
                  {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : '建立帳號'}
                </button>
              </div>
          ...
          {mode === 'forgot' && (
            <div>
              <h2 className="mb-1 text-base font-semibold text-gray-900">忘記密碼</h2>
              <p className="mb-4 text-sm text-gray-500">輸入您的 Email，我們將寄送重設連結</p>
              ...
              {forgotSent ? (
                ...
                <form onSubmit={forgotForm.handleSubmit(handleForgot)} noValidate>
                  ...
                  >
                    {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : '發送重設連結'}
                  </button>
                </form>
              )}
        ...
        <p className="mt-6 text-center text-xs text-gray-400">
          登入即代表您同意我們的
          <a href="/privacy" className="underline hover:text-gray-600">隱私政策</a>
          與
          <a href="/terms" className="underline hover:text-gray-600">使用條款</a>
        </p>
      </div>
    </main>
  )
