...
import { updateFaqSchema } from '@/lib/validations/about'
...
export const PUT = withAdmin(async (req: NextRequest, ctx, _user) => {
  ...
  if (error || !data) {
    console.error('[PUT /api/admin/faqs/[id]]', error?.message)
    return apiError('更新失敗', 500, 'UPDATE_FAILED')
  }

  return apiSuccess({ faq: data as unknown as Record<string, unknown> })
})
...
export const DELETE = withAdmin(async (_req, ctx, _user) => {
  ...
  if (error || !data) {
    console.error('[DELETE /api/admin/faqs/[id]]', error?.message)
